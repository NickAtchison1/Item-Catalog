...
def gconnect():
    ...
    # Verify that the access tpken is used for the intended user
    ...
    # Store the access toen in the session for later user
    ...
    # See if user exsist, if not create a new one
...
def deleteCategory(category_id):
    ...
    if request.method == 'POST':
        ...
        flash('%s Successfuly Deleted' % categoryToDelete.name)
        session.commit()
...
def newItem(category_id):
    ...
    if request.method == 'POST':
        newItem = Item(name=request.form['name'], description=request.form['description'],
                       price=request.form['price'], category_d=category_id)
        session.add(newItem)
        session.commit()
        flash('New Category %s Item Successfully Created' % (newItem.name))
        return redirect(url_for('showItem.html', category_id=category_id))
    else:
